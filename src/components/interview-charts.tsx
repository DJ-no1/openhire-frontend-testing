...
export function UniversalScoresChart({ data }: UniversalScoresChartProps) {
  ...
  const averageScore =
    Object.values(safeData).reduce((sum, score) => sum + score, 0) /
    Object.values(safeData).length;
        ...
        <CardContent className="pb-2">
          <div className="flex items-center justify-center h-[180px] text-gray-500">
            <div className="text-center">
              ...
              <p className="text-xs">
                Scores will appear here once assessment is complete
              </p>
            </div>
          </div>
        </CardContent>
...
export function IndustryCompetencyChart({
  data,
  industryType = "General",
}: IndustryCompetencyChartProps) {
  const chartData =
        ...
        <CardContent className="pb-2">
          <div className="flex items-center justify-center h-[180px] text-gray-500">
            <div className="text-center">
              ...
              <p className="text-xs font-medium">
                No domain-specific competencies assessed
              </p>
...
export function ConversationEngagementChart({
  conversationLog,
  overallScore = 75,
}: ConversationEngagementChartProps) {
  ...
  const generateEngagementData = () => {
    ...
    return conversationLog.map((entry, index) => ({
      ...
      engagement: entry.engagement_score
        ? Math.round(entry.engagement_score * 10)
        : 0, // Convert to 0-100 scale
      timestamp: index + 1,
    ...
    return (
      <Card className="border border-slate-200 shadow-sm">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs text-gray-600">
            Engagement level throughout the interview
          </CardDescription>
        </CardHeader>
        <CardContent className="pb-2">
          <div className="flex items-center justify-center h-[180px] text-gray-500">
            <div className="text-center">
              ...
              <p className="text-xs">
                Engagement metrics will appear here after interview completion
              </p>
            </div>
          </div>
        </CardContent>
  ...
  const trend =
    chartData.length > 1
      ? ((chartData[chartData.length - 1].engagement -
          chartData[0].engagement) /
          Math.max(1, chartData[0].engagement)) *
        100
      : 0;

  const avgEngagement =
    chartData.reduce((sum, item) => sum + item.engagement, 0) /
    chartData.length;
  ...
  return (
    <Card className="border border-slate-200 shadow-sm">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs text-gray-600">
          Real-time engagement throughout the interview
        </CardDescription>
      </CardHeader>
      <CardContent className="pb-2">
      ...
      <CardFooter className="flex-col items-start gap-1 text-xs pt-2">
        ...
        <div className="text-gray-500 leading-none">
          Based on response quality and interaction patterns ({chartData.length}{" "}
          questions)
        </div>
      </CardFooter>
    </Card>
