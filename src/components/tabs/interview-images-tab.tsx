...
export function InterviewImagesTab({ artifact, applicationDetails }: InterviewImagesTabProps) {
    ...
    const parseImageUrls = (imageUrlString: string): ProcessedImage[] => {
        ...
        return urls.map((url, index) => ({
            ...
            metadata: {
                ...
                format: url.includes('.png') ? 'PNG' : url.includes('.jpg') ? 'JPG' : 'Image',
                capturedAt: new Date(Date.now() - (urls.length - index) * 120000).toISOString()
            }
        }));
    };
    ...
    const handleDownloadImage = async (image: ProcessedImage) => {
        try {
            ...
            link.download = `interview_image_${image.id}.${image.metadata?.format?.toLowerCase() || 'jpg'}`;
    ...
    const ImagePreview = ({ image, isGridView }: { image: ProcessedImage; isGridView?: boolean }) => {
        const isBroken = brokenImages.has(image.id);

        if (isBroken) {
            return (
                <div className={`${isGridView ? 'h-48' : 'h-24 w-24'} bg-gray-100 border border-gray-300 rounded-lg flex items-center justify-center`}>
        ...
        return (
            <img
                ...
                className={`${isGridView ? 'h-48 w-full' : 'h-24 w-24'} object-cover rounded-lg border border-gray-200`}
                ...
                <Card className="border-2 border-dashed border-gray-300">
                    <CardContent className="p-12 text-center">
                        ...
                        <p className="text-gray-500 mb-4">
                            No images were captured during this interview session, or the image data is not available.
                        </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="border-2 border-purple-200 bg-purple-50/30">
                    <CardContent className="p-6 text-center">
                        ...
                        <div className="text-3xl font-bold text-purple-700">{images.length}</div>
                ...
                <Card className="border-2 border-green-200 bg-green-50/30">
                    <CardContent className="p-6 text-center">
                        ...
                        <div className="text-lg font-bold text-green-700">
                            {images.length > 0 ? formatTimestamp(images[0].timestamp).split(' ')[0] : 'N/A'}
                        </div>
                        <div className="text-sm text-gray-600">Capture Date</div>
                    </CardContent>
                </Card>
                                ...
                                <div key={image.id} className="space-y-3">
                                                    ...
                                                    <DialogContent className="max-w-4xl">
                                                        ...
                                                        <div className="space-y-4">
                                                            ...
                                                            <div className="flex items-center justify-between text-sm text-gray-600">
                                                                <span>Captured: {formatTimestamp(image.timestamp)}</span>
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center justify-between text-sm">
                                            <span className="font-medium">Image {image.id.replace('img_', '')}</span>
